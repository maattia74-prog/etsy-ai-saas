...
export class AIClient {
  ...
  constructor() {
    ...
    if (process.env.TOGETHER_API_KEY) {
      this.together = new Together({ auth: process.env.TOGETHER_API_KEY });
    }
    ...
    try {
      if (provider === 'groq' && this.groq) {
        ...
        return {
          ...
          provider: 'groq',
          costUsd: this.calculateCost('groq', usage.prompt_tokens ?? 0, usage.completion_tokens ?? 0),
        };
      }
      ...
      if (provider === 'together' && this.together) {
        ...
        return {
          ...
          tokensUsed: usage.total_tokens ?? 0,
          provider: 'together',
          costUsd: this.calculateCost('together', usage.prompt_tokens ?? 0, usage.completion_tokens ?? 0),
        };
      }
      ...
      if (provider === 'openai' && this.openai) {
        ...
        return {
          ...
          tokensUsed: usage.total_tokens ?? 0,
          provider: 'openai',
          costUsd: this.calculateCost('openai', usage.prompt_tokens ?? 0, usage.completion_tokens ?? 0),
        };
      }
